...
# 全局变量
MYNum="vless28"
SINGBOX_DIR="/opt/sing-box/$MYNum"
SINGBOX_CONFIG_DIR="/etc/sing-box/$MYNum"
SINGBOX_LOG_DIR="/var/log/sing-box/$MYNum"
SINGBOX_VERSION=""
SINGBOX_ARCH=""
SYSTEM=""
PUBLIC_IP=""
...
read_vless_config() {
    ...
    local config_file="/opt/$MYNum/xray_node_info.json"
    
    if [[ ! -f "$config_file" ]]; then
        log_error "未找到配置文件: $config_file"
        echo -e "${YELLOW}请确保文件存在：${NC}"
        echo -e "  - /opt/$MYNum/xray_node_info.json"
        exit 1
    fi
...
download_singbox() {
    ...
    local download_file="sing-box-1.12.12-linux-amd64.tar.gz"
    local download_url="https://soft-1257202154.cos.ap-guangzhou.myqcloud.com/${download_file}"
    ...
    cat > "$SINGBOX_CONFIG_DIR/config.json" << EOF
{
    ...
    "route": {
        ...
        "rules": [
            {
                "inbound": ["socks-in-$MYNum"],
                "outbound": "vless-out-$MYNum"
            }
        ],
        "final": "direct-$MYNum"
    }
...
create_systemd_service() {
    ...
    cat > /etc/systemd/system/sing-box-relay-$MYNum.service << EOF
[Unit]
...
EOF
    ...
    systemctl enable sing-box-relay-$MYNum
...
start_singbox_service() {
    ...
    # 停止可能存在的旧服务
    systemctl stop sing-box-relay-$MYNum 2>/dev/null || true
    
    # 启动服务
    systemctl start sing-box-relay-$MYNum
    sleep 5
    
    if systemctl is-active --quiet sing-box-relay-$MYNum; then
        log_info "sing-box中转服务启动成功"
    else
        log_error "sing-box中转服务启动失败"
        echo "服务状态："
        systemctl status sing-box-relay-$MYNum --no-pager
        echo ""
        echo "错误日志："
        journalctl -u sing-box-relay-$MYNum --no-pager -n 20
        echo ""
        echo "手动测试命令："
...
save_config_json() {
    ...
    local config_file="/opt/$MYNum/vless_to_socks5_config.json"
    ...
    cat > "$config_file" << EOF
{
    ...
    "config_files": {
        ...
        "service_file": "/etc/systemd/system/sing-box-relay-$MYNum.service",
...
show_config_summary() {
    ...
    echo -e "  ${CYAN}中转配置：${YELLOW}/opt/$MYNum/vless_to_socks5_config.json${NC}"
    echo -e "  ${CYAN}日志目录：${YELLOW}$SINGBOX_LOG_DIR${NC}\n"
    
    echo -e "${WHITE}${BOLD}🛠️ 服务管理：${NC}"
    echo -e "  ${CYAN}启动服务：${YELLOW}systemctl start sing-box-relay-$MYNum${NC}"
    echo -e "  ${CYAN}停止服务：${YELLOW}systemctl stop sing-box-relay-$MYNum${NC}"
    echo -e "  ${CYAN}重启服务：${YELLOW}systemctl restart sing-box-relay-$MYNum${NC}"
    echo -e "  ${CYAN}查看状态：${YELLOW}systemctl status sing-box-relay-$MYNum${NC}"
    echo -e "  ${CYAN}查看日志：${YELLOW}journalctl -u sing-box-relay-$MYNum -f${NC}"
    ...
    if systemctl is-active --quiet sing-box-relay-$MYNum; then
        echo -e "  ${GREEN}${ICON_SUCCESS} sing-box-relay-$MYNum 服务运行正常${NC}"
    else
        echo -e "  ${RED}${ICON_ERROR} sing-box-relay-$MYNum 服务未运行${NC}"
    fi
    ...
    echo -e "     journalctl -u sing-box-relay-$MYNum -f --no-pager"
...
main() {
    ...
    # 生成配置JSON并上传
    if [[ -f "/opt/$MYNum/vless_to_socks5_config.json" ]]; then
        # 读取现有配置
        CONFIG_JSON=$(cat "/opt/$MYNum/vless_to_socks5_config.json")

        # 保存导出配置到文件
        CONFIG_FILE="/opt/$MYNum/vless_to_socks5_config_export.json"
    ...
    echo -e "${WHITE}完整配置信息保存在: ${YELLOW}/opt/$MYNum/vless_to_socks5_config.json${NC}"
